...
| **IndexDB** | `lib/storage/index_db.go` | series 索引、`generateTSID` | 查询路径 |
...
|------|--------------|----------------------------|
| #1 | `1736935200000` | `2026_01` |
| #7 | `1738569600000` | `2026_02` |
...
| 磁盘上出现含 5 个文件的 part 目录 | ❌ | flush 由 `startPendingRowsFlusher` 等后台触发（第 3 篇） |
...
4. **本篇终点**：`partition.AddRows` → **rawRows 内存**；磁盘 part 见第 3 篇。
  
---
  
## 12. 下篇预告
  
**第 3 篇：rawRows 缓冲与 flush → inmemory part**
  
将延续同一 7 个点，说明：
  
- `pendingRowsFlushInterval`、`maxRawRowsPerShard` 如何触发 flush；  
- 一次 flush **是否**、**何时**产生 inmemory part；  
- 与磁盘 5 文件的关系（第 4 篇）。
